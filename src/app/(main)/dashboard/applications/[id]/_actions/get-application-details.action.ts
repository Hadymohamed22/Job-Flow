...
import { ApplicationDetailsResponse } from "../_types/application-details-response";
...
export async function getApplicationDetailsAction(id: string) {
  try {
    ...
    const payload: ApplicationDetailsResponse = await res.json();

    if ("error" in payload) {
      throw new Error(
        payload.message ||
          "Failed to get application details , Try again later !",
      );
    }
